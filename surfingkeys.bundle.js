...
`;
  ...
  var openTeamUnityUpdateStatusSheets = () => {
    if (window.location.hostname.includes("docs")) {
      window.location.href = "https://docs.google.com/spreadsheets/d/1Olck6WYQ-lRHvnApmjVAb9DxXaBDJeOsiqvJJqrkTZ4/edit?gid=1080140336#gid=1080140336";
    } else {
      window.open(
  ...
  api.mapkey(
    "gys",
    "\u{1F511} Copy GitHub Repo SSH clone URL with hints",
    function() {
      ...
      const getSshUrl = (el) => {
        ...
        if (match && !["orgs", "topics", "collections", "features", "settings"].includes(
          match[1]
        )) {
          const owner2 = match[1];
          const repo2 = match[2].replace(/\.git$/, "");
          return `git@github.com:${owner2}/${repo2}.git`;
        }
      ...
      if (hasRepoElements) {
        hintCreated = api.Hints.create(repoSelector, (el) => {
          ...
          if (sshUrl) {
            ...
            api.Front.showBanner(`\u{1F511} Copied SSH: ${sshUrl}`);
          } else {
    ...
    const replaceText = (node) => {
      if (node.nodeType === Node.TEXT_NODE) {
        ...
        if (!node.parentElement?.closest("script") && !node.parentElement?.closest("style") && !node.parentElement?.getAttribute("href")) {
          newText = newText.replace(/\bChatGPT\b/gi, "SJ Pulse \u{1F680}").replace(/\bChatGPT-\d\b/gi, "SJ Pulse").replace(/\bAsk anything\b/gi, "Pulse Query \u{1FAC0}");
        }
