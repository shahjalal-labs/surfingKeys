  ...
  var require_githubUrlOpener = __commonJS({
    "src/modules/github/githubUrlOpener.js"(exports, module) {
      ...
      api.mapkey("gro", "Github own Repositories", function () {
        window.open(
          "https://github.com/shahjalal-labs?tab=repositories",
          "_blank",
        );
      });
      githubUrlOpener(
...
#sk_status, #sk_find {
      ...
      Node.prototype.insertBefore = function (newNode, referenceNode) {
        if (
          ...
          newNode.setAttribute(
            "href",
            customFaviconUrl || createCustomFavicon(),
          );
        }
    ...
    const replaceText = (node) => {
      if (node.nodeType === Node.TEXT_NODE) {
        ...
        if (
          ...
          newText = newText
            .replace(/\bChatGPT\b/gi, "SJ Pulse \u{1F680}")
            .replace(/\bChatGPT-\d\b/gi, "SJ Pulse")
            .replace(/Copy code/gi, "Yank \u{1F4CB}")
            .replace(/\bCopy\b/gi, "Yank")
            .replace(/\bAsk anything\b/gi, "Pulse Query \u{1FAC0}");
        }
  ...
  `;
    ...
    const originalSetLanguage =
      languageToggler.setLanguage.bind(languageToggler);
